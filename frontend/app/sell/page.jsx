...
function SellPageInner() {
  ...
  // ever consider creating a new one. Mirrors the same "latest listing is
  // a draft" rule the beforeCreate lifecycle enforces, via
  // GET /auction-items/:userId/current-draft-id (see the backend
  // controller's currentDraftId handler).
  const fetchCurrentDraftIdFromBackend = useCallback(async (forUserId) => {
    try {
      const res = await apiClient.get(`/auction-items/${forUserId}/current-draft-id`);
      const payload = res?.data || res;
      return payload?.draftId || null;
    } catch (err) {
      console.error('Failed to resolve current draft id from backend:', err.status, err.message);
      return null;
    }
  }, []);
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      // ── Draft mode ──
      // Step 1: localStorage first — this is the fast path and needs no
      // network round trip if it's still valid.
      const cachedDocId = typeof window !== 'undefined' ? localStorage.getItem(STORAGE_KEYS.DRAFT_DOCUMENT_ID) : null;
      if (cachedDocId) {
        try {
          // FIXED: this used to omit `populate[seller]`, so entity.seller
      ...
      // Step 2: no usable cached id — ask the backend whether this seller
      // already has a draft in progress before ever considering creating
      // a new one. This is what actually prevents the "new draft on every
      // soft navigation" problem: localStorage can be empty (cleared,
      // different device/browser, etc.) while a real draft still exists
      // server-side, and the backend is the source of truth for that.
      const backendDraftId = await fetchCurrentDraftIdFromBackend(userId);
      if (cancelled) return;
      if (backendDraftId) {
        try {
          await loadExistingDraftById(backendDraftId);
          if (!cancelled) setPhase('ready');
        } catch (err) {
          if (!cancelled) { setErrorMsg(err.message || 'Failed to load your existing draft'); setPhase('error'); }
        }
        return;
      }

      // Step 3: genuinely no draft anywhere — create one. Still guarded by
      // the creation lock below in case another concurrent init (Strict
      // Mode double-invoke, a fast double-click into Sell, etc.) reaches
      // this same point at the same time.
      if (!tryAcquireDraftCreationLock()) {
        const publishedDocId = await waitForCachedDraftId();
        if (cancelled) return;
        if (publishedDocId) {
